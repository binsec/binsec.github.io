---
layout: post
title:  "SP 2020 - Extended Abstract"
date:   2020-10-06
...
pdf: https://arxiv.org/abs/1912.08788
---
...
Take for instance the following `password_checker` which compares a secret
...
The function iterates through each character of the string and returns
`false` as soon as there is a mismatch.  Let `PasSw0rD` be the value
of the password. Notice that the attempt `Myguess?` will fail after
only one iteration of the loop whereas the attempt `Password` will
fail after three iterations of the loop. An attacker *measure the
execution time* of the function with different values of `guess` to
...
as OpenSSL, Libsodium, etc., use the **constant-time programming**
discipline.  It consists in designing the program in such a way that
...
For instance, our `password_checker` function becomes:
``` c
...
```
Compiled with `clang -m32 -march=i386 -O0` this function is compiled
to constant-time code whereas `clang -m32 -march=i386 -O3`, it is
...
a program, contrary to safety properties which guarantee the absence
of bugs along *one execution*. Therefore, common bug-finding tools for
safety are not directly applicable to constant-time and we need tools
...
- Using Binsec/Rel, we perform an **extensive analysis of CT at
